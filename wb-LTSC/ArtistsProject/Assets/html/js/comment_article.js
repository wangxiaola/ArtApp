...
app.controller('MainCtrl', ['$scope', '$sce', '$timeout', '$location', '$anchorScroll',
  function($scope, $sce, $timeout, $location, $anchorScroll) {
    ...
    $scope.upvoteArticle = function(is_praise) {
      if ($scope.articleDetail.upvoteStatus == 1) {
        return;
      } else {
        $scope.articleDetail.upvoteStatus = 1;
      }
      var data = {
        'is_praise': is_praise
      };
      ...
      jsbridge.callNativeForResult("page", "upvoteArticle", data, function(resp) {
        ...
        $scope.safeApply(function() {
          if (resp.is_praise) {
            $scope.articleDetail.praise++;
            $scope.articleDetail.is_praise = true;
          } else {
            $scope.articleDetail.praise--;
            $scope.articleDetail.is_praise = false;
          }
          $scope.articleDetail.upvoteStatus = 0;
        });
      });
    }
    ...
    function initArticleData() {
      var articleDetail = $scope.data.articleDetail;
      $scope.articleDetail = articleDetail;
      if (articleDetail.is_praise) {
        $scope.articleDetail.upvoteStatus = 2; // upvoteStatus 0 可点击，1 locked, 2已点赞
      } else {
        $scope.articleDetail.upvoteStatus = 0;
      }
      // $scope.articleDetail.content = $sce.trustAsHtml(articleDetail.post_content);
    }
    ...
    function updateData(data) {
      $scope.data = data;
      initArticleData(data.articleDetail);
