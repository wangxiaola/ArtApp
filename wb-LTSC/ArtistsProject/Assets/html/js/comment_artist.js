...
app.controller('MainCtrl', ['$scope', '$sce', '$timeout', '$location', '$anchorScroll',
  function($scope, $sce, $timeout, $location, $anchorScroll) {
    ...
    $scope.upvoteArticle = function(is_praise) {
      if ($scope.personDetail.upvoteStatus == 1) {
        return;
      } else {
        $scope.personDetail.upvoteStatus = 1;
      }
      var data = {
        'is_praise': is_praise
      };
      ...
      jsbridge.callNativeForResult("page", "upvoteArticle", data, function(resp) {
        ...
        $scope.safeApply(function() {
          if (resp.is_praise) {
            $scope.personDetail.praise++;
            $scope.personDetail.is_praise = true;
          } else {
            $scope.personDetail.praise--;
            $scope.personDetail.is_praise = false;
          }
          $scope.personDetail.upvoteStatus = 0;
        });
      });
    }
    ...
    // 初始化文章相关的数据
    function initArtistData() {
      var personDetail = $scope.data.personDetail;
      $scope.personDetail = personDetail;
      if (personDetail.is_praise) {
        $scope.personDetail.upvoteStatus = 2; // upvoteStatus 0 可点击，1 locked, 2已点赞
      } else {
        $scope.personDetail.upvoteStatus = 0;
      }
      // $scope.personDetail.content = $sce.trustAsHtml(personDetail.post_content);
    }
    ...
    function updateData(data) {
      $scope.data = data;
      initArtistData(data.personDetail);
