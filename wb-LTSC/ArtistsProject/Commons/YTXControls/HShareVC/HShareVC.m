...
- (void)createUI
{
    ...
    //NSArray* lstImages = @[ @"icon_share1", @"icon_share2", @"icon_share4",@"icon_share7", @"icon_share8", @"icon_share9", @"icon_share10" ];
    //NSArray*  lstTitles = @[ @"微信好友", @"朋友圈", @"新浪微博", @"短信", @"发送邮件", @"复制链接", @"新窗口打开" ];
       NSArray*  lstTitles = @[ @"微信好友", @"朋友圈", @"新浪微博", @"短信", @"复制链接", @"新窗口打开" ];
    
    NSArray *secondRowImageArray = @[@"icon_share110", @"icon_share13", @"icon_share15", @"icon_share12", @"icon_share11"];
    NSArray *secondRowtitleArray = @[@"编辑", @"删除", @"置顶",@"收藏", @"举报" ];
    ...
    for (int i = 0; i < secondRowImageArray.count; i++)
    {
        HButton* btnShare = [HButton new];
        if ([Global sharedInstance].isgroup.integerValue != 1){
            if (i<3) {
               btnShare.frame = CGRectMake(0, kScreenW/320*10, 0, 0);
            }else{
               btnShare.frame = CGRectMake((i-3) * btnWidth, kScreenW/320*10, btnWidth, btnWidth);
            }
        }else{
        btnShare.frame = CGRectMake(i * btnWidth, kScreenW/320*10, btnWidth, btnWidth);
        }
        [btnShare setImage:[UIImage imageNamed:secondRowImageArray[i]] forState:UIControlStateNormal];
        btnShare.imageTextAlignment = HButtonImageTextAlignmentVertical;
        [btnShare setTitle:secondRowtitleArray[i] forState:UIControlStateNormal];
        [btnShare setTitleColor:kTitleColor forState:UIControlStateNormal];
        [btnShare.titleLabel setFont:[[Global sharedInstance] fontWithSize:12]];
        btnShare.tag = i + 20;
        [btnShare addTarget:self action:@selector(customBtn_Click:) forControlEvents:UIControlEventTouchUpInside];
        [secconView addSubview:btnShare];
    }
...
- (void)displaySMSComposerSheetPhoneNumber:(NSString*)phoneNumber
{
    ...
    picker.body = [NSString stringWithFormat:@"%@", self.msg.desc];
