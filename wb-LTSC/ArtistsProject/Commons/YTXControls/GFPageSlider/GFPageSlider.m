...
static const CGFloat kIndicatorLineHeight = 2.0f; // 下划线指示器的高度
...
- (void)initPartitionLine {
    _partitionLine = [[UIView alloc] initWithFrame:CGRectMake(0, _menuHeight - kPartitionLineHeight, kSelfViewWidth / _menuNumberPerPage * _pageCount/2, kPartitionLineHeight)];
    _partitionLine.backgroundColor = [UIColor colorWithRed:218.0f / 255.0 green:218.0f / 255.0 blue:218.0f / 255.0 alpha:1];
    _partitionLine.hidden = YES;
...
- (void)setupTitles:(NSArray *)titles withFormerPageCount:(int)formerPageCount {
    for (int i = formerPageCount; i < _pageCount ; ++ i) {
        ...
        if (self.menuTitleColor){
            if (i == 0) {
                [button setTitleColor:_indicatorLineColor forState:UIControlStateNormal];
            }else{
...
{
    UIButton *menuButton=(UIButton*)[_menuScrollView viewWithTag:(selectedPageIndex+1000)];
    [self moveIndicatorWithButton:menuButton animated:NO];
    [self changePageWithSelectedButton:menuButton animated:NO];
}
...
- (void)clickMenuButton:(UIButton *)menuButton {
    [self moveIndicatorWithButton:menuButton animated:YES];
    [self changePageWithSelectedButton:menuButton animated:YES];
}

- (void)moveIndicatorWithButton:(UIButton *)menuButton animated:(BOOL)animated{
    if (animated){
    ...
    if (_pageCount > _menuNumberPerPage) { // 只有当翻页的页面数比菜单栏名目可见菜单多的时候才需要在滑动过程中移动菜单栏
        ...
        if (menuButton.tag >= 1001 && menuButton.tag <= (1001 + (_pageCount - _menuNumberPerPage)) ) {
            [_menuScrollView setContentOffset:CGPointMake((menuButton.tag - 1001) * (kSelfViewWidth / _menuNumberPerPage), 0) animated:animated];
        }
        // 而当菜单栏移动到底的时候，剩余每个MenuButton选中时的偏移距离都统一设置为(_pageCount - _menuNumberPerPage)个MenuButton宽度
            ...
            [_menuScrollView setContentOffset:CGPointMake((_pageCount - _menuNumberPerPage) * (kSelfViewWidth / _menuNumberPerPage), 0) animated:animated];
        }
    ...
    if([menuButton isKindOfClass:[UIButton class]]) {
        if (self.menuTitleColor){
            [_formerButton setTitleColor:self.menuTitleColor forState:UIControlStateNormal];
        }else{
...
- (void)changePageWithSelectedButton:(UIButton *)menuButton animated:(BOOL)animated{
    _currentPage = menuButton.tag - 1000;
    [_contentScrollView setContentOffset:CGPointMake(_currentPage * kSelfViewWidth, 0) animated:animated];
    if (self.didPageChangedBlock){
        self.didPageChangedBlock(self, _currentPage);
    }
}
