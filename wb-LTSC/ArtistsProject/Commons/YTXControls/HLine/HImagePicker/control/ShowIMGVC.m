...
#define WIDTH_PIC kScreenW
#define HEIGHT_PIC    kScreenH
...
- (void)createBar{
    ...
    UIView *toolBarBGView = [[UIView alloc]initWithFrame:CGRectMake(0, kScreenH-44-64, kScreenW, 44)];
...
- (void)createCollectionView{
    ...
    self.collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, WIDTH_PIC, HEIGHT_PIC - 44 - 64) collectionViewLayout:flowLayout];
...
- (UICollectionViewCell*)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectedBlock = ^(BOOL select){
        ...
        if (select) {
            if (self.selectedArray.count>self.maxNumberOfImage-1) {
                cell.bImageChoose=NO;
                [self showErrorHUDWithTitle:[NSString stringWithFormat:@"不能超出%d张",self.maxNumberOfImage] SubTitle:nil Complete:nil];
                return ;
            }
        }
