...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"通讯录中邀请";
...
-(NSMutableArray *)getBook
{
        ...
        {
            NSLog(@"用户未授权提示");
            [self showErrorHUDWithTitle:@"通讯录未授权" SubTitle:@"请前往<设置>中设置权限" Complete:nil];
        }
...
-(void)sendSmsMessageWithPhoneNumber:(NSString *)phoneNumber
{
    if([MFMessageComposeViewController canSendText])
    {
        if (phoneNumber.length == 0) {
            UIAlertView *msgbox = [[UIAlertView alloc] initWithTitle:jekPrompt message:@"手机号码有误" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:nil];
            [msgbox show];
        }else{
