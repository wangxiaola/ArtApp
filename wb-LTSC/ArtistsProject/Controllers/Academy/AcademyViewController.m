...
#import "AcademyTableCell.h"
#import "AcademyHeaderView.h"
@interface AcademyViewController ()<UITableViewDataSource, UITableViewDelegate, SDCycleScrollViewDelegate>
{
    UIView *menuView;
    BOOL isShowMenu;
...
{
    AcademyHeaderView *view = [[[NSBundle mainBundle]loadNibNamed:@"AcademyHeaderView"owner:self options:nil]lastObject];
    if (section == 0) {
        view.title.text = @"好客推荐";
    }if (section == 1) {
        view.title.text = @"立即上课";
    }else {
        view.title.text = @"即将开课";
    }
    return view;
}
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    AcademyTableCell *academyCell = [tableView dequeueReusableCellWithIdentifier:@"AcademyTableCell"];
    return academyCell;
}
