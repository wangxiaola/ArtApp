...
@interface PublishDongtaiVC () <UITableViewDelegate, UITableViewDataSource, AVAudioPlayerDelegate> {
    HView* viewBottom;
    HTextView* txt;
...
@property(nonatomic)AVPlayer* play;
@end

@implementation PublishDongtaiVC
@synthesize state;
...
- (void)viewDidLoad
{
    ...
    self.scrollView.backgroundColor = ColorHex(@"f6f6f6");
    self.automaticallyAdjustsScrollViewInsets = NO;
    if ([_topictype isEqualToString:@"4"]) {
        ...
        _selectedType = @"重要展览";
        self.navigationItem.title = @"重要展览";
    ...
    UIBarButtonItem* rightBar = [[UIBarButtonItem alloc] initWithTitle:@"发布" style:UIBarButtonItemStyleDone target:self action:@selector(rightBar_Click)];
...
- (void)createView:(UIView*)contentView
{
    ...
    [txt mas_makeConstraints:^(MASConstraintMaker* make) {
        ...
        if ([_topictype isEqualToString:@"4"] ||[_topictype isEqualToString:@"6"] || [_topictype isEqualToString:@"8"] || [_topictype isEqualToString:@"13"] || [_topictype isEqualToString:@"14"]|| [_topictype isEqualToString:@"15"] || [_topictype isEqualToString:@"16"] || [_topictype isEqualToString:@"18"] || [_topictype isEqualToString:@"17"] || [_topictype isEqualToString:@"9"]) {
    ...
    __weak typeof(arrayImageSave)blockArr = arrayImageSave;
    [imgSel setSelectDelBtnCilck:^(NSInteger iNumber){
        if (blockArr.count>0){
            if (blockArr.count>iNumber) {
                [blockArr removeObjectAtIndex:iNumber];
    ...
    if ([_topictype isEqualToString:@"7"]) {
        ...
        [typeView addSubview:line1];
        [line1 mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.equalTo(typeView);
            make.height.mas_equalTo(0.8);
            make.bottom.equalTo(typeView.mas_bottom).offset(0);
        }];
    ...
    } else if ([_topictype isEqualToString:@"4"] ||[_topictype isEqualToString:@"6"] || [_topictype isEqualToString:@"8"] || [_topictype isEqualToString:@"13"] || [_topictype isEqualToString:@"14"]|| [_topictype isEqualToString:@"15"] || [_topictype isEqualToString:@"16"] || [_topictype isEqualToString:@"18"] || [_topictype isEqualToString:@"17"] || [_topictype isEqualToString:@"9"]) {
        ...
        [_tableView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            }else if ([_topictype isEqualToString:@"4"]) {
...
- (void)wanchengChuanshu
{
    ...
    if ([state isEqualToString:@"1"]) {
    ...
    }else if ([_topictype isEqualToString:@"4"]) {// 商品
        YTXPublishViewCell *cell0 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
        gtype = _selectTypeId;
        gtypename = _selectedType;
        ...
        price = [cell2 subtitle];
        YTXPublishViewCell *cell3 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:3 inSection:0]];
        yunfei = [cell3 subtitle];
        
        YTXPublishViewCell *cell4 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:4 inSection:0]];
        kucun = [cell4 subtitle];
        ...
        YTXPublishViewCell *cell6 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:6 inSection:0]];
        _people = [cell6 subtitle];
        
        YTXPublishViewCell *cell7 = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:7 inSection:0]];
        message = [cell7 subtitle];
    ...
    NSLog(@"gtype= %@,gtypename = %@,topictitle = %@, price = %@, yunfei = %@, kucun = %@, huiyuan = %@, people = %@, message = %@",gtype,gtypename,topictitle, price,yunfei,kucun,_huiyuan,_people,_message);
...
//    return;
    ...
    NSMutableDictionary* dicAll = @{ @"topictype" : _topictype ? : @"",
                                     @"gtype":gtype?:@"",
                                     @"gtypename":gtypename?:@"",
                                     @"sellprice":[NSString stringWithFormat:@"%.2f",[price floatValue]*100]?:@"",
                                     @"yunfei":yunfei?:@"",
                                     @"kucun":kucun?:@"",
                                     @"huiyuan":_huiyuan?:@"2",
...
- (void)rightBar_Click
{
    ...
    if ([_topictype isEqualToString:@"6"]) {
        ...
        if (_selectedType.length == 0) {
            ...
            return;
//        }if (_price.length == 0) {
//            [self showErrorHUDWithTitle:@"请输入商品价格" SubTitle:nil Complete:nil];
//            return;
//        }if (_yunfei.length == 0) {
//            [self showErrorHUDWithTitle:@"请输入商品运费" SubTitle:nil Complete:nil];
//            return;
//        }if (_kucun.length == 0) {
//            [self showErrorHUDWithTitle:@"请输入商品库存" SubTitle:nil Complete:nil];
//            return;
//        }if (_people.length == 0) {
//            [self showErrorHUDWithTitle:@"请输入艺术家" SubTitle:nil Complete:nil];
//            return;
...
- (void)tableView:(UITableView*)tableView didSelectRowAtIndexPath:(NSIndexPath*)indexPath
{
    if ([tableView isEqual:_tableView]) {
        ...
        if ([[cell title] isEqualToString:@"会员专属"]) {
            return;
        }
        if ([[cell title] isEqualToString:@"系统类目"]) {
            ...
            [classVC setSaveBtnCilck:^(NSString *name, NSString *nameId) {
                ...
                self.selectTypeId = nameId;
                [weak__tableView reloadData];
                [self.view setNeedsLayout];
                [self.view layoutIfNeeded];
            }];
            [self.navigationController pushViewController:classVC animated:YES];
        }else if ([[cell title] isEqualToString:@"分类"]) {
            @weakify(self);
            YTXCustomTypeViewController *customTypeVC = [[YTXCustomTypeViewController alloc] init];
            if ([_topictype isEqualToString:@"6"]) {
            ...
            customTypeVC.didSelectedString = ^(NSString *string, CUSTOM_TYPE customType) {
                ...
                } else if ([string isEqualToString:@"重要展览"]) {
                    self.topictype = @"8";
                    [weak__tableView mas_updateConstraints:^(MASConstraintMaker *make) {
                        make.height.mas_equalTo(280);
                    }];
            ...
            if ([[cell title] isEqualToString:@"尺寸"]){
                if (_longstr.length>0&&![_longstr isEqualToString:@"0"]) {
                    customTypeVC.longstr = _longstr;
                }
                if (_height.length>0&&![_height isEqualToString:@"0"]) {
                    customTypeVC.height = _height;
                }
                if (_width.length>0&&![_width isEqualToString:@"0"]) {
            ...
            if (viewCell.subtitle.length > 0) {
                ...
                if (([[cell title] isEqualToString:@"价格"]&&[_price isEqualToString:@"价格为空时显示询价"]) ||([[cell title] isEqualToString:@"运费"]&&[_yunfei isEqualToString:@"默认为0，卖家承担运费"])) {
            ...
            customTypeVC.resultBlock = ^(NSString *result) {
                ...
                } else if ([self.topictype isEqualToString:@"4"]) {
                    if (indexPath.row == 1) {
                        _name = result;
                    } else if (indexPath.row == 2) {
                        _price = result;
    ...
    if ([tabRecorder isEqual:tableView]) {
        ...
        NSDictionary* dic = _arrayRecorderSave[indexPath.row];
        if (isBofang) {
            if ([dic containsObjectForKey:@"1"]) {
                NSURL* urlAudio = [dic objectForKey:@"1"];
            ...
            }else if ([dic containsObjectForKey:@"url"]){
                NSURL* urlAudio = [NSURL URLWithString:[dic objectForKey:@"url"]];
            AVPlayer*  tempPlay = [[AVPlayer alloc]initWithURL:urlAudio];
                self.play = tempPlay;
                [self.play pause];
...
//                    NSMutableAttributedString *titleString = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"价格为空是显示询价 %@",_price]];
//                    [titleString addAttribute:NSForegroundColorAttributeName value:[UIColor hexChangeFloat:@"fea900"]range:NSMakeRange(0, 9)];
//                    cell.subtitleLabel.attributedText = titleString;
                    
                    [cell setSubtitle:_price];
                ...
                case 4:
                {
                    ...
                    [cell setSubtitle:_kucun];
                    break;
                }case 5:
                {
                    [cell setTitle:@"会员专属"];
                    cell.accessoryType = UITableViewCellAccessoryNone;
                    cell.huiyuan.hidden = NO;
                    if ([_huiyuan isEqualToString:@"1"]) {
                        cell.huiyuan.on = YES;
                    }if ([_huiyuan isEqualToString:@"2"]) {
                        cell.huiyuan.on = NO;
                    }
//                    [cell setSubtitle:_message];
                    break;
