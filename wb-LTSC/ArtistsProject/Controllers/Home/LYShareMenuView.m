...
// 每行有4个
#define kShareMenuRowItemCount 3
//总共有2列
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIImageView * bacImageViews =[[UIImageView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
         bacImageViews.image = [UIImage imageNamed:@"背景"];        
        [self addSubview:bacImageViews];
        [self setup_];
    }
    return self;
}
...
- (void)showShareMenuView{
    ...
    UIImageView * bacImageView =[[UIImageView alloc] initWithFrame:CGRectMake(0, T_WIDTH(120), kScreenW, T_WIDTH(90))];
    bacImageView.image = [UIImage imageNamed:@"背景文字"];
...
- (UIScrollView *)shareMenuScrollView{
    if (!_shareMenuScrollView) {
        ...
        shareMenuScrollView.backgroundColor = [UIColor clearColor];
...
- (UIButton *)cancelButton{
    if (!_cancelButton) {
        ...
        _cancelButton.frame = CGRectMake(0, CGRectGetHeight(self.backgroundMenuView.frame) -T_WIDTH(kCancelbuttonHeight), CGRectGetWidth(self.backgroundMenuView.frame), T_WIDTH(kCancelbuttonHeight));
        ...
        _cancelButton.titleLabel.font = ART_FONT(ARTFONT_OE);
        [_cancelButton addTarget:self action:@selector(dismissShareMenuView) forControlEvents:UIControlEventTouchUpInside];
        _cancelButton.backgroundColor = [UIColor clearColor];
        UIImageView * cance =[[UIImageView alloc] initWithFrame:CGRectMake((kScreenW-21)/2, T_WIDTH(kCancelbuttonHeight)/2-10, 21, 21)];
        cance.image =[UIImage imageNamed:@"guanbi"];
        [_cancelButton addSubview:cance];
    }
