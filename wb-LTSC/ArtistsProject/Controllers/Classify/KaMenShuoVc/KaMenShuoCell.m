...
-(void)setArtTableViewCellDicValue:(NSDictionary *)dic{
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        [_baseImage sd_setImageWithUrlStr:[NSString stringWithFormat:@"%@!120a",dic[@"user"][@"avatar"]] tempTmage:@"temp_Default_headProtrait"];
        _seeNums.text = dic[@"user"][@"username"];
    }else{
        _seeNums.hidden = YES;
...
-(void)tapClick{
    NSDictionary* dic = _recentCellDic[@"user"];
...
-(void)btnMore_Click{
    ...
    NSString* arrStr = model.photos;
    NSString* imgUrl = [arrStr componentsSeparatedByString:@","][0];
    if (imgUrl.length>0) {
        shareVc.shareimage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:imgUrl]]];
    }else{
        shareVc.shareimage = [UIImage imageNamed:@"temp_Default_headProtrait"];
    }
    
    shareVc.sharetitle=[NSString stringWithFormat:@"「%@文字」%@ ",model.user.username,
                        model.topictitle];
    shareVc.sharedes=@" ";
