...
@property (nonatomic, strong) HLabel *messageLabel;
...
+ (void)initialize
{
    ...
    appearance.messageColor = ColorHex(@"666666");
    ...
    appearance.destructiveButtonColor = ColorHex(@"e1c360");
...
- (void)addButtonWithTitle:(NSString *)title type:(SIAlertViewButtonType)type handler:(SIAlertViewHandler)handler
{
	...
	item.title = title;
	item.type = type;
    item.isAutoDismiss=YES;
	item.action = handler;
	[self.items addObject:item];
}

- (void)addButtonWithTitle:(NSString *)title type:(SIAlertViewButtonType)type isAutoDismiss:(BOOL)isAutoDismiss handler:(SIAlertViewAndItemHandler)handler
{
    SIAlertItem *item = [[SIAlertItem alloc] init];
    item.title = title;
    item.type = type;
    item.isAutoDismiss=isAutoDismiss;
    item.action2 = handler;
    [self.items addObject:item];
}
...
- (void)validateLayout
{
    ...
    if (self.messageLabel) {
        ...
        lineTop.frame=CGRectMake(0, y, self.containerView.bounds.size.width, 1);
        lineBottom.frame=CGRectMake(0, y+height, self.containerView.bounds.size.width, 1);
        y += height;
    }
    if (self.customView){
        [self.customView setFrame:CGRectMake(0, y, self.containerView.bounds.size.width, self.customView.frame.size.height)];
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2 && self.buttonsListStyle == SIAlertViewButtonsListStyleNormal) {
            CGFloat width = 90;// (self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2 - GAP) * 0.5;
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(CONTENT_PADDING_LEFT, y, width, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(self.containerView.bounds.size.width - CONTENT_PADDING_LEFT - width, y, width, BUTTON_HEIGHT);
        } else {
...
- (void)updateMessageLabel
{
    if (self.message) {
        if (!self.messageLabel) {
            self.messageLabel = [[HLabel alloc] initWithFrame:self.bounds];
            ...
            lineTop=[[HLine alloc] initWithFrame:self.bounds];
            lineTop.lineColor=ColorHex(@"d5d5d5");
            lineTop.lineStyle=UILineStyleHorizon;
            lineTop.lineWidth=1;
            lineTop.backgroundColor=[UIColor clearColor];
            [self.containerView addSubview:lineTop];
            
            lineBottom=[[HLine alloc] initWithFrame:self.bounds];
            lineBottom.lineColor=ColorHex(@"d5d5d5");
            lineBottom.lineStyle=UILineStyleHorizon;
            lineBottom.lineWidth=1;
            lineBottom.backgroundColor=[UIColor clearColor];
            [self.containerView addSubview:lineBottom];
        }
        self.messageLabel.text = self.message;
    } else {
...
- (void)buttonAction:(UIButton *)button
{
    ...
    if (item.action2){
        item.action2(self,item);
        if (item.isAutoDismiss==NO) return;
    }
