...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _headerTitleColor = FSCalendarStandardYearTitleColor;
...
#warning textColor Night
        //添加夜间模式适配
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"APP_NIGHT_MODE"]&&[[NSUserDefaults standardUserDefaults] boolForKey:@"APP_NIGHT_MODE"] == YES) {
            _titleColors[@(FSCalendarCellStateNormal)]      = [UIColor lightGrayColor];
        }
        _titleColors[@(FSCalendarCellStateSelected)]    = [UIColor whiteColor];
        _titleColors[@(FSCalendarCellStateDisabled)]    = [UIColor grayColor];
        _titleColors[@(FSCalendarCellStatePlaceholder)] = [UIColor darkGrayColor];
