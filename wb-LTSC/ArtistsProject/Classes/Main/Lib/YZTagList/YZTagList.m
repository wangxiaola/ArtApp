...
- (CGFloat)tagListH
{
    ...
    YZTagButton *button = self.tagButtons.lastObject;
    return CGRectGetMaxY(button.frame) + _tagMargin;
}
...
- (void)addTags:(NSArray *)tagStrs
{
    ...
    for (int i = 0; i < tagStrs.count; i++) {
        NSString *tagStr = tagStrs[i];
        [self addTag:tagStr index:i];
    }
}
// 添加标签
- (void)addTag:(NSString *)tagStr index:(int)index
{
...
//    tagButton.lineBreakMode = NSLineBreakByWordWrapping;
    ...
    if (self.changeBtnBGColor) {
        if (isNightMode) {
            
            [tagButton setTitleColor:[UIColor colorWithRed:152 / 255.0  green:152 / 255.0 blue:152 / 255.0 alpha:1] forState:UIControlStateNormal];
        }else {
        
            [tagButton setTitleColor:[UIColor colorWithRed:106 / 255.0  green:106 / 255.0 blue:106 / 255.0 alpha:1] forState:UIControlStateNormal];
        }
    }else {
        
        [tagButton setTitleColor:[UIColor colorWithRed:98 / 255.0  green:98 / 255.0 blue:98 / 255.0 alpha:1] forState:UIControlStateNormal];
    }
    
    if (!self.changeBtnH && !self.changeBtnBGColor) {
        
        [tagButton setTitleColor:[UIColor colorWithRed:181 / 255.0  green:27 / 255.0 blue:32 / 255.0 alpha:1] forState:UIControlStateSelected];
    }else if (self.changeBtnBGColor) {
        
        [tagButton setTitleColor:[UIColor colorWithRed:255 / 255.0  green:255 / 255.0 blue:255 / 255.0 alpha:1] forState:UIControlStateSelected];
    }
...
- (void)clickTag:(UIButton *)button
{
    if (_clickTagBlock) {
        ...
        if (!self.changeBtnH) {
            ...
            if (button.selected && self.changeBtnBGColor) {
                
                [button setBackgroundColor:[UIColor colorWithRed:229 / 255.0 green:40 / 255.0 blue:46 / 255.0 alpha:1]];
            }else {
...
- (void)setupTagButtonRegularFrame:(UIButton *)tagButton
{
    ...
    CGFloat btnH = _tagSize.height + 20;
    ...
    CGFloat btnY = _tagMargin + 10 + row * (btnH + margin);
...
- (void)setupTagButtonCustomFrame:(UIButton *)tagButton preButton:(UIButton *)preButton extreMargin:(BOOL)extreMargin
{
    ...
    if (btnW >= self.bounds.size.width) {
        btnW = self.bounds.size.width - 10;
    }
