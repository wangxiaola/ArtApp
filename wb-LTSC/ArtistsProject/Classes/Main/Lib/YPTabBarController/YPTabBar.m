...
//        item.titleColor = self.itemTitleColor;
        [item dk_setTitleColorPicker:DKColorPickerWithRGB(0x000000, 0x989898) forState:UIControlStateNormal];
...
- (void)updateSubViewsWhenParentScrollViewScroll:(UIScrollView *)scrollView {
    ...
    if (self.itemColorChangeFollowContentScroll) {
        ...
        if (isNightMode) {
            ...
            self.itemTitleColor = [UIColor colorWithHex:0x989898];
        }else {
        
            self.itemTitleColor = [UIColor colorWithHex:0x000000];
        }
