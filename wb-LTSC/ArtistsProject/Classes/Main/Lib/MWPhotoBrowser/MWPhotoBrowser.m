...
- (void)viewDidLoad {
    ...
    if (self.displayActionButton) {
        UIImage * moreImage = [UIImage imageNamed:@"detailpage_item_more"];
        _actionButton = [[UIBarButtonItem alloc] initWithImage:moreImage style:UIBarButtonItemStyleDone target:self action:@selector(actionButtonPressed:)];
...
- (void)longPress{
    ...
    if ([_delegate respondsToSelector:@selector(photoBrowser:longPressedForPhotoAtIndex:image:)]) {
        [_delegate photoBrowser:self longPressedForPhotoAtIndex:_currentPageIndex image:image];
    }
}
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:image:)]) {
            
            // Let delegate handle things
            [self.delegate photoBrowser:self actionButtonPressedForPhotoAtIndex:_currentPageIndex image:[self imageForPhoto:photo]];
